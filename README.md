...
> **为什么需要第 3 步？** Claude Code 插件系统支持 skills、agents、hooks、MCP servers，但**不支持 rules**。`rules/sdlc-workflow.md` 是 SDLC 工作流的感知层，需要每次会话自动加载，只能通过 `~/.claude/rules/` 目录实现。
